...
_Built (#59)._ The other three draw the thing they are for. This one's product
happens on **somebody else's screen** — it presents the device to a host as a
BLE mouse — and that one fact decides the whole page.

```text
┌────────────────────────────────────────────────┐
│  [BATTERY]   Air Mouse - connected             │  no network named: there
...
**The header carries the connection, not the name.** Everywhere else the title
says which app you are in, which you can also see. Here you cannot see the
thing the app is doing, and **a mouse nobody has connected to looks exactly
like a mouse that is working** — so the one question the screen can usefully
answer is whether a host is listening, and that goes where the eye already
goes. It is `ui.title(s)`, and nothing else about this page needs the platform's
permission.

**A standard screen, not `bare`.** The instinct for a full-screen toy is screen
1; it would be wrong here, because the header _is_ what this app has to show.
`bare` buys a panel with nothing reserved, and this page's whole content is the
reserved strip.

**Three states, three sentences.** `off`, `advertising`, `connected` are not two
booleans to be combined — "waiting to be picked up" and "not offering itself at
all" are different things to tell somebody, and an app that had to derive one
from the other would eventually get it wrong. And a device with **no radio for
this at all** says so in words and stops, rather than sitting there telling the
user to pair with something that is not being advertised.

**Wi-Fi off is the driver's promise, not the app's.** The two radios share one
2.4 GHz front end, and unlike a scan — a few seconds, asked for repeatedly — a
mouse is held for as long as the app is open. So the association is parked when
the mouse starts and restored when it goes. An app asked to remember that would
...
touch of the tree is a 153,600-byte blit. A page that rewrote the same word
every frame would spend the whole app redrawing a screen nobody is looking at,
while the cursor it exists to move stuttered.

**There is no teardown, deliberately.** B, long B and a fault all end an app,
and only one of them is a path an app can write code on. The platform takes the
mouse down instead, as an invariant: nothing that is not running is a mouse.
**An app must never be the only thing that remembers to give a radio back.**
