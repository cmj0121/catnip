...
static const char CATNIP_GRID_LUA[] =
    ...
    /* A pinned app is drawn in the primary role so the ones already on the ring
     * stand out from the ones only here - the marker a user reads to see what a
     * pin did. */
    "    cells[i] = ui.label{ id = 'grid' .. i, text = name, image = ids[i],\n"
    "                         icon = 'placeholder', disabled = not ready[i],\n"
    "                         style = pinned[i] and 'primary' or 'body' }\n"
    "  end\n"
    "  local sel = open\n"
    "  local list\n"
    ...
    "    end,\n"
    "    on_next = function()\n"
    "      if sel < #cells then sel = sel + 1; list.selected = sel end\n"
    "    end,\n"
    ...
    "      elseif ready[sel] then __catnip_grid_pick(sel) end\n"
    "    end,\n"
    /* Long-A pins or unpins the selected app. Never reached by touch. */
    "    on_options = function(self, i) __catnip_grid_pin(sel) end }\n"
                          ...
                          bool fs_ready, const char *unpinned)
{
    ...
    lua_pushinteger(L, 1); /* open on the first app */
