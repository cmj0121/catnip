...
static void exp_pin_write(uint8_t pin, bool high)
{
    ...
    if (high) out |= (uint8_t)(1u << pin);
    else out &= (uint8_t)~(1u << pin);
...
void setup()
{
    ...
    if (rd(ADDR_PMU, 0x12, &rails)) {
        ...
        if (!(rails & 0x02)) {
            wr(ADDR_PMU, 0x27, 104);
            wr(ADDR_PMU, 0x12, rails | 0x02);
            delay(50);
        }
    } else {
...
void loop()
{
    ...
    Serial.printf(
        "[poc] %-5s | pmu=%d rails=0x%02X | exp=%d cfg=0x%02X out=0x%02X in=0x%02X\n",
