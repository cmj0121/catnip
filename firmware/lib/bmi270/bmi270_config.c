/*
 * bmi270_config.c - the BMI270's configuration image, verbatim from Bosch.
 *
 * A BMI270 powers up in a configuration mode and produces no data at all
 * until this image has been written into it; INTERNAL_STATUS reads not_init
 * until then, which is exactly what this board's part was measured doing.
 * The image is not something that can be derived or written by hand - it is
 * Bosch's, and it is vendored here rather than fetched at build time so the
 * firmware builds offline and always against a known version.
 *
 * Source: https://github.com/boschsensortec/BMI270-Sensor-API, bmi270.c,
 * the bmi270_config_file[] array. BSD-3-Clause; see LICENSE beside this file.
 * Copied unmodified: 8192 bytes.
 */
